...
- At this pinned commit, [`erts_init_scheduling()` and
  `erts_start_schedulers()`](https://github.com/erlang/otp/blob/e07fd07837e5aa845657f5fa340637121e451d47/erts/emulator/beam/erl_process.c)
  retain a normal scheduler, at least one dirty CPU scheduler, one dirty I/O
  scheduler, an auxiliary thread, and a poll thread even at the smallest
  proposed scheduler settings. [`erl_async.c`](https://github.com/erlang/otp/blob/e07fd07837e5aa845657f5fa340637121e451d47/erts/emulator/beam/erl_async.c)
  and [`erl_trace.c`](https://github.com/erlang/otp/blob/e07fd07837e5aa845657f5fa340637121e451d47/erts/emulator/beam/erl_trace.c)
  add async and system-message-dispatch work. These are seven candidate
  ERTS/POSIX roles for the target census, not proof of seven newly created
  pthreads or browser Worker hosts: startup topology, the thread running
  `main`, build options, and allocator activity affect the physical count.
...
- [Processes, schedulers, signals, and timers component](../20-notes/components/processes-schedulers-signals-and-timers.md)
- [Memory, garbage collection, and shared-state component](../20-notes/components/memory-garbage-collection-and-shared-state.md)
